...
const getCarInfoByIndex = (inventory, index) => {
  ...
  return `This is a car ${car.car_make} ${car.car_model}`;
};
...
const sortCarInventory = inventory => {
  const res = inventory.sort((carA, carB) => {
...
const getModelYears = inventory => {
  const years = new Array(inventory.length);
 ...
 */
const getOlderCars = (inventory, maxYear) => {
  const olderCars = [];
  for (const car in inventory) {
    if (inventory[car].car_year <= maxYear) olderCars.push(inventory[car]);
  }
  return olderCars;
  // ? not sure if array.from can work here since it only has an iterable, not a filter
};
...
const getGermanCars = inventory => {
  ...
  const germanCars = [];
  for (const car of inventory) {
    // * are we allowed to use `.includes`?
    if (germanBrands.includes(car.car_make)) germanCars.push(car);
  }
  return germanCars;
};
